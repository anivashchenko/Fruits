...
struct BasketList: View {
    ...
    @State var isListOneOn: Bool = true
    ...
    func delete(indexSet: IndexSet) {
        let oldList = newList
        newList.remove(atOffsets: indexSet)
    ...
    func move(indices: IndexSet, newOffset: Int) {
        ...
        for (index, var food) in newList.enumerated() {
            food.order = index + 1
            
            if food.typeFood == .fruits {
                let foodIndex = modelData.fruits.firstIndex { $0.id == food.id }!
                modelData.fruits[foodIndex].order = index + 1
            } else if food.typeFood == .vegies {
                let foodIndex = modelData.vegies.firstIndex { $0.id == food.id }!
                modelData.vegies[foodIndex].order = index + 1
            } else {
                let foodIndex = modelData.berries.firstIndex { $0.id == food.id }!
                modelData.berries[foodIndex].order = index + 1
            }
        }
    }
    ...
    func getList() {
        ...
        let sortedOrderedFood = compactOrderedFood.sorted(by: { $0.order < $1.order })
        
        var unique: [Food] = []
        for food in sortedOrderedFood {
        ...
        //        var lastOrder = 0
        //        for food in sortedOrderedFood {
        //            if food.order == lastOrder {
        //                newList.removeLast()
        //            }
        //                newList.append(food)
        //                lastOrder = food.order
        //        }
    }
